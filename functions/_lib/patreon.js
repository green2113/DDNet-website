...
function utf8ToBinaryString(value) {
  return bytesToBinaryString(encoder.encode(String(value || '')));
}
...
export function hmacMd5Hex(key, message) {
  const keyRaw = utf8ToBinaryString(key);
  const msgRaw = utf8ToBinaryString(message);
  return rstr2hex(rstrHmacMd5(keyRaw, msgRaw));
}

export function md5Hex(value) {
  return rstr2hex(rstrMd5(utf8ToBinaryString(value)));
}
