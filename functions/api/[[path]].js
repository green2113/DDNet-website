...
async function ensureCasinoTrailTable(env) {
  ...
  `).run();
  await env.DB.prepare(`
    CREATE INDEX IF NOT EXISTS idx_casino_trail_items_account_id
    ON casino_trail_items(account_id)
  `).run();
  await env.DB.prepare(`
    CREATE INDEX IF NOT EXISTS idx_casino_trail_items_expires_unix
    ON casino_trail_items(expires_unix)
  `).run();
  await env.DB.prepare(`
...
async function pruneCasinoTrailItems(env, accountId, nowUnix) {
  await env.DB.prepare(`
    DELETE FROM casino_trail_items
    WHERE account_id = ? AND expires_unix <= ?
  `).bind(accountId, nowUnix).run();
}

async function loadCasinoTrailInventory(env, accountId, nowUnix) {
  const result = await env.DB.prepare(`
    SELECT id, mode, days, expires_unix, equipped
...
async function handleGameCasinoTrails(context) {
  ...
  const nowUnix = Math.floor(Date.now() / 1000);
  await pruneCasinoTrailItems(env, accountId, nowUnix);
  ...
  if(mode === 'buy') {
    ...
    if(!CASINO_TRAIL_ALLOWED_DAYS.has(days)) {
      return json({ ok: false, message: 'Invalid trail duration' }, 400);
    }

    const owned = await env.DB.prepare(`
      SELECT id
      FROM casino_trail_items
      WHERE account_id = ? AND mode = ? AND expires_unix > ?
      LIMIT 1
    `).bind(accountId, trailMode, nowUnix).first();
    if(owned) {
      const inventory = await loadCasinoTrailInventory(env, accountId, nowUnix);
      return json({ ok: false, code: 'ALREADY_OWNED', accountId, inventory }, 409);
    }

    await env.DB.prepare(`
      INSERT INTO casino_trail_items (account_id, mode, days, expires_unix, equipped, created_at)
      VALUES (?, ?, ?, ?, 0, ?)
    `).bind(accountId, trailMode, days, nowUnix + days * 86400, nowIso()).run();
